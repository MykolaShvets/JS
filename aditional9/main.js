...
for (const user of usersList) {
    ...
    box.append(id, name, username, email, address, phone, website, company);
    ...
    for (const point in user.address){
        let addressLi = document.createElement('li');
        address.append(addressLi);
        addressLi.innerText =point + ': ' + user.address[point];
        if(typeof user.address[point] === 'object' ){
            addressLi.innerText =point + ': ';
            let geoList = document.createElement('ul');
            addressLi.append(geoList);
            for (const item in user.address[point]){
                let geoLi = document.createElement('li');
                geoList.append(geoLi);
                geoLi.innerText = user.address[point][item];
            }
    ...
    for (const key in user.company){
        ...
        company.append(companyLi);
        companyLi.innerText = user.company[key];
    }
